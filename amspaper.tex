...
roughly follows the theoretical isotropic inverse cascade spectrum (\ref{eq:Kolmogorov_Kraichnan_spectrum_energy}) with the -5/3 slope
from the forcing scale all the way to slightly less than the energy
halting scale, and (b) the zonal energy spectrum $\mathcal{E}_{Z}(k)$
...
Instead, it appears robust that the total spectrum 
$\mathcal{E}(k)$ start to steepen above $\mathcal{E}_{eddy}(k)$
near the crossover scale even for runs with $\Pi=0.6$. This may confirm
the relevance of crossover scale as the scale where Rossby waves
start to become significant. In the enstrophy cascade range
