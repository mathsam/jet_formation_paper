...
(\ref{eq:Kolmogorov_Kraichnan_spectrum_energy}) with the -5/3 slope
from the forcing scale all the way to slightly less than the energy
halting scale, and (b) the ZKE spectrum $\mathcal{E}_{Z}(k)$
is much steeper than $\mathcal{E}_{eddy}(k)$ and roughly follows a -5 slope
...
isotropic turbulence with the $-5/3$ slope no longer exists but the EKE
spectrum following the $-3$ slope extends from the enstrophy cascade 
branch ($k>k_{f}$) all the way into the inverse cascade branch ($k_{r}<k<k_{f}$). 
The $-3$ slope in the EKE spectrum confirms our scaling of turbulence
frequency in (\ref{eq:eddy_turnover_freq_direct_enstrophy_cascade}) 
for the enstrophy cascade branch is robust. The lack of $-5/3$ slope
may suggest that the energy cascade is no longer local but is also 
driven by the shear of the jets in a similar way as the enstrophy 
non-local cascade as we discussed in the end of the Theory Section.
...
crossover scale ($k_\varepsilon$ or $k_\ega$ in Fig. \ref{EKE_KE_spectra_illustrate}).
This may confirm that the crossover scale moves to smaller than forcing
scale for the case $\Pi<1$. Theoretically, we expect the ZKE spectrum 
(\ref{eq:zonostrophic_spectrum}) with -5 slope may no longer hold 
when $\Pi<1$. In practice, however, it is difficult to tell the 
difference between the runs here. One reason is that there
are only a couple of jets in the runs so that shape of ZKE spectrum,
which describes the statistics of jets, cannot be resolved very accurately.

For $\Pi<1$, the flow lacks a near-isotropic inverse cascade range as 
suggested by the lack of $-5/3$ EKE spectrum slope. Instead, eddies at 
